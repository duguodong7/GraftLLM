...
class GPTQ:
    ...
    def __init__(self, layer, quant_type,observe=False):
        self.layer = layer
        self.dev = self.layer.V.device
        self.quant_type = quant_type
        # W = layer.weight.data.clone()
        
        W = layer.V.T.data.clone() if self.quant_type == 'V' else layer.U.data.clone()
    ...
    def print_loss(self, name, q_weight, weight_error, timecost):
        ...
        if self.quant_type == 'V':
            self.layer.V.data = q_weight.reshape(self.layer.V.data.T.shape).to(self.layer.V.data.dtype).T
        else:
    ...
    def fasterquant(self, blocksize=128, percdamp=.01, groupsize=-1, actorder=False, name=''):
        ...
        W = self.layer.V.data.T.clone() if self.quant_type == 'V' else self.layer.U.data.clone()
        if isinstance(self.layer, nn.Conv2d):
            W = W.flatten(1)
        if isinstance(self.layer, transformers.Conv1D):
            W = W.t()
